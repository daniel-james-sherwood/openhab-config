...
floors = [
    ...
    { "ftag": "FF", "fname": "Upstairs", "switch": 2, "rooms": [
        ...
        #{ "rtag": "B3", "rname": "Alex's Bedroom",    "icon": "boy_3",          "address": "OEQ1421625" },
...
#                     'String\t{_rtag}_AMD\t"Alexa Mode{_posn}"\t<switch>\t({rtag_AMD}_g{rtag})\t["homekit:HeatingCoolingMode"]',
                     ...
                     'Group:Number\tVP_{_rtag}\t"{rname}{_posn}"\t<pressure>\t({VP_rtag}gVP_{ftag}, gVP)',
...
def write_switch_rule(fh, tag):
    fh.write('\n'.join(['rule "gVP_{tag}->P_{tag}"',
                        ...
                        '\tTime cron "0 0/1 * * * ?"',
                        'then',
                        '\tvar newState = if(gVP.state >= 79 && gVP_{tag}.state >= 10) ON else OFF',
